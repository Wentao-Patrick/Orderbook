...
# Script role: full-parameter bivariate Hawkes LR test with FIB bootstrap and diagnostics.
SCRIPT_DIR = Path(__file__).resolve().parent
HF_ORDERFLOW_DIR = SCRIPT_DIR.parent
DATA_INPUT_DIR = HF_ORDERFLOW_DIR / "data" / "input"
RESULTS_DIR = HF_ORDERFLOW_DIR / "results"
FIGURES_DIR = HF_ORDERFLOW_DIR / "figures"
...
def save_rescaling_outputs(v: np.ndarray, prefix: str, results_dir: Path, figures_dir: Path):
    ...
    fig1 = figures_dir / f"{prefix}_hist_exp.png"
    ...
    fig2 = figures_dir / f"{prefix}_qq_exp.png"
    ...
    fig3 = figures_dir / f"{prefix}_qq_unif.png"
    plt.figure(figsize=(5, 5))
    u_sorted = np.sort(u)
    ...
    return str(out_csv), str(fig1), str(fig2), str(fig3), summary, ks_exp, ks_unif
...
def main():
    ...
    ap.add_argument("--buy_csv", default=str(DATA_INPUT_DIR / "sanofi_hawkes_buy_times.csv"))
    ap.add_argument("--sell_csv", default=str(DATA_INPUT_DIR / "sanofi_hawkes_sell_times.csv"))
    ...
    ap.add_argument("--figures_dir", default=str(FIGURES_DIR))
    args = ap.parse_args()
    ...
    if not args.skip_diagnostics:
        ...
        out1 = save_rescaling_outputs(v1, prefix="half1_fullopt", results_dir=results_dir, figures_dir=figures_dir)
        out2 = save_rescaling_outputs(v2, prefix="half2_fullopt", results_dir=results_dir, figures_dir=figures_dir)
    ...
    lr_star_csv = results_dir / "LR_star_FIB_fullopt.csv"
    np.savetxt(lr_star_csv, LR_star, delimiter=",")

    print("\n=== FIB bootstrap (FULL MLE refit) ===")
    print(f"B = {args.B}  seed = {args.seed}")
    print(f"p_boot (FIB) = {p_boot:.6f}")
    print(f"Saved: {lr_star_csv}")
    ...
    plt.grid(alpha=0.3)
    plt.tight_layout()
    lr_hist_png = figures_dir / "LR_star_FIB_fullopt_hist.png"
    plt.savefig(lr_hist_png, dpi=160)
    plt.close()
    print(f"Saved: {lr_hist_png}")
