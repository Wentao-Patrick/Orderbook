...
# Script role: build buy/sell Hawkes event-time CSVs from decoded trade records.
SCRIPT_DIR = Path(__file__).resolve().parent
HF_ORDERFLOW_DIR = SCRIPT_DIR.parent
PROJECT_ROOT = HF_ORDERFLOW_DIR.parent.parent
DEFAULT_INPUT_CSV = PROJECT_ROOT / "decoded_full_trade_information.csv"
DEFAULT_BUY_CSV = HF_ORDERFLOW_DIR / "data" / "input" / "sanofi_hawkes_buy_times.csv"
DEFAULT_SELL_CSV = HF_ORDERFLOW_DIR / "data" / "input" / "sanofi_hawkes_sell_times.csv"
...
def export_hawkes_sequences(
    ...
    Path(out_buy).parent.mkdir(parents=True, exist_ok=True)
    Path(out_sell).parent.mkdir(parents=True, exist_ok=True)
...
def main() -> None:
    ...
    ap.add_argument("--out_buy", default=str(DEFAULT_BUY_CSV))
    ap.add_argument("--out_sell", default=str(DEFAULT_SELL_CSV))
