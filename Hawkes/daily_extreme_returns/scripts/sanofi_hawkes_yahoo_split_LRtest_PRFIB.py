...
# Script role: daily extreme-return Hawkes LR stability test with PRFIB bootstrap.
SCRIPT_DIR = Path(__file__).resolve().parent
DAILY_DIR = SCRIPT_DIR.parent
DATA_INPUT_DIR = DAILY_DIR / "data" / "input"
...
# If the local CSV exists, it is used first.
PRICE_CSV: Optional[str] = str(DATA_INPUT_DIR / "sanofi_SAN.PA_yahoo_daily.csv")
# If local CSV is missing or PRICE_CSV is None, fall back to yfinance.
USE_YFINANCE_IF_NO_CSV = True

# Diagnostics output
OUT_DIR = str(FIGURES_DIR)
RESULTS_OUT_DIR = str(RESULTS_DIR)
SAVE_FIGS = True
...
def main():
    ...
    Path(RESULTS_OUT_DIR).mkdir(parents=True, exist_ok=True)

    # Load prices
    local_csv = Path(PRICE_CSV) if PRICE_CSV is not None else None
    if local_csv is not None and local_csv.exists():
        price = read_price_csv(str(local_csv))
        print(f"[OK] Read prices from CSV: {local_csv} ({len(price)} rows)")
    else:
        if not USE_YFINANCE_IF_NO_CSV:
            raise RuntimeError("Local PRICE_CSV is unavailable and USE_YFINANCE_IF_NO_CSV is False.")
    ...
    out_csv = f"{RESULTS_OUT_DIR}/sanofi_yahoo_events_from_returns.csv"
