...
def align_true_pred(D_true, D_pred):
    """Return same-timestamp aligned true/pred dataframe."""
    aligned = pd.concat(
        [D_true.rename('true'), D_pred.rename('pred')],
        axis=1,
        join='inner',
    ).dropna()
    return aligned
...
def compute_metrics(D_test, D_pred, D_full=None):
    """Compute level MSE and direction hit ratio on aligned timestamps."""
    aligned = align_true_pred(D_test, D_pred)
    if aligned.empty:
        return {'mse_D': float('nan'), 'hit_ratio': float('nan'), 'n': 0, 'n_hit': 0}

    mse_D = float(np.mean((aligned['true'] - aligned['pred']) ** 2))

    base = D_full if D_full is not None else D_test
    D_t = base.shift(1).reindex(aligned.index)
    valid = D_t.notna()
    if not valid.any():
        hr = float('nan')
        n_hit = 0
    else:
        delta_real = aligned.loc[valid, 'true'] - D_t.loc[valid]
        delta_pred = aligned.loc[valid, 'pred'] - D_t.loc[valid]
        hr = float(np.mean(np.sign(delta_real) == np.sign(delta_pred)))
        n_hit = int(valid.sum())
...
def plot_results(D_full, D_pred_full, outdir=OUT_DIR):
    outdir.mkdir(parents=True, exist_ok=True)
    aligned = align_true_pred(D_full, D_pred_full)
    if aligned.empty:
        return []
    ...
    plt.plot(aligned.index, aligned['true'], label='D_true')
    plt.plot(aligned.index, aligned['pred'], label='D_pred')
    plt.legend()
    ...
    plt.scatter(aligned['pred'], aligned['true'], s=8)
    mx = max(aligned['pred'].max(), aligned['true'].max())
    ...
    # 4 Delta scatter
    D_t = D_full.shift(1).reindex(idx)
    valid = D_t.notna()
    delta_real = aligned.loc[valid, 'true'] - D_t.loc[valid]
    delta_pred = aligned.loc[valid, 'pred'] - D_t.loc[valid]
    plt.figure(figsize=(5,5))
    if len(delta_pred) > 0:
        ...
        plt.text(0.5, 0.5, 'Not enough points for delta scatter', ha='center', va='center')
        plt.axis('off')
    ...
    e_non_na = e.dropna()
    if len(e_non_na) > 2:
        lags = min(40, len(e_non_na) - 1)
        plot_acf(e_non_na, lags=lags)
    else:
        plt.text(0.5, 0.5, 'Not enough points for ACF', ha='center', va='center')
        plt.axis('off')
    ...
    aligned_out = outdir / 'aligned_true_pred.csv'
    aligned.to_csv(aligned_out, index_label='timestamp')

    return [p1, p2, p3, p4, p5, aligned_out]
...
def pipeline_demo():
    ...
    # compute metrics
    metrics = compute_metrics(D_test, D_pred, D_full=D)
