    ...
    def apply(self, u: Update) -> None:
        ...
        if a == 1:
            ...
            if not self._is_depth_eligible(u):
                # we do not put it on depth curve; just count it in market bucket
                if u.order_type == 1:
...
class OrderBookReplayer:
    ...
    def animate_day_mp4_with_ffmpeg(
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            # Use scale filter to ensure dimensions are even (required by h264)
            cmd = [
                ffmpeg_path,
                "-y",
                "-framerate", str(fps),
    ...
    def _match_and_place_order(self, u: Update):
        """
        A mini-matching engine for aggressive orders (Market, MTL).
        It matches against the resting book and if the order is an MTL with
        quantity left over, it places it on the book as a limit order.
        """
        if u.qty is None or u.qty <= 0 or u.side is None:
            return

        aggressor_qty = u.qty
        aggressor_side = u.side
        
        # 1. Find opposite side orders and sort them for matching
        opposite_side = 2 if aggressor_side == 1 else 1
        
        # A buy aggressor wants the lowest price sell (ascending sort)
        # A sell aggressor wants the highest price buy (descending sort)
        sort_reverse = (aggressor_side == 2) 

        resting_orders = sorted(
            [(pid, o) for pid, o in self.orders.items() if o.side == opposite_side and o.price > 0],
        ...
        # 2. Iterate and match
        for priority, resting_order in resting_orders:
            if aggressor_qty <= 0:
                break

            # Price check for matching
            can_match = False
            if aggressor_side == 1 and resting_order.price > 0: # Buy aggressor
                # For now, assume market orders cross the spread. A real engine would have price limits.
                can_match = True
            elif aggressor_side == 2 and resting_order.price > 0: # Sell aggressor
                can_match = True

            if can_match:
                traded_qty = min(aggressor_qty, resting_order.qty)
                
                aggressor_qty -= traded_qty
                resting_order.qty -= traded_qty
                last_traded_price = resting_order.price

                # If resting order is fully filled, remove it
                if resting_order.qty <= 0:
                    self.orders.pop(priority, None)
                else: # Partially filled, update the order in place
                    self.orders[priority] = resting_order


        # 3. Handle MTL order leftovers
        if u.order_type == 6 and aggressor_qty > 0 and last_traded_price is not None:
            # This is the core MTL logic: place remainder as a limit order
            # at the last traded price.
            if u.priority is not None:
                self.orders[int(u.priority)] = Order(
                    side=int(u.side),
                    price=last_traded_price,
                    qty=aggressor_qty,
                    order_type=2 # It's now a Limit order
                )
        # For regular Market orders (type 1), any leftover quantity is simply discarded
        # as it has no price to rest on the book. This is a common simplification.
        elif u.order_type == 1 and aggressor_qty > 0:
             # We can account for the remaining market order quantity if needed
             if u.side == 1:
                 self.market_bucket_qty_buy += aggressor_qty
             elif u.side == 2:
                 self.market_bucket_qty_sell += aggressor_qty
...
if __name__ == "__main__":
    order_update_path = "/Users/charles/Documents/Tâches/Mathématiques/EA/EA_recherche/euronextparis/EuronextParis/EuronextParis_20191001_FR0000120578/FR0000120578/OrderUpdate_20191001_FR0000120578.csv"
    intervals_path = "/Users/charles/Documents/Tâches/Mathématiques/EA/EA_recherche/msc_decoded/merged_intervals.csv"

    # 1) build replayer
    rep = OrderBookReplayer(order_update_path, include_market_in_depth=False)

    # 2) Find continuous trading session for animation and plotting
    try:
        intervals_df = pd.read_csv(intervals_path)
        continuous_session = intervals_df[intervals_df["book_state"].str.contains("Continuous", na=False)]
        continuous_session = continuous_session.loc[continuous_session['duration_s'].idxmax()]
