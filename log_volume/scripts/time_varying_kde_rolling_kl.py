...
def side_tagged_path(path: Path, side: str) -> Path:
    """Ensure output filename contains side tag (_bid / _ask)."""
    side_l = str(side).strip().lower()
    stem = path.stem
    suffix = path.suffix
    if re.search(rf"(?:^|[_-]){re.escape(side_l)}(?:[_-]|$)", stem, flags=re.IGNORECASE):
        return path
    return path.with_name(f"{stem}_{side_l}{suffix}")
...
def parse_args() -> argparse.Namespace:
    ...
    default_output = results_dir / "rolling_kl_time_varying_kde.png"
    default_video_output = results_dir / "log_volume_1s_video.mp4"
    default_four_pdf_output = results_dir / "pdf_10_12_14_16.png"
...
def main() -> None:
    args = parse_args()
    # Always mark output artifacts with side to avoid overwriting and ambiguity.
